...
def get_angle(landmark, eps=1e-6):
    ...
    # counter clockwise (the bigger y-value, the lower in the image coords)
...
def rotate_bbox(bbox, d_angle, cx, cy, h, w):
    ...
    corners = np.array(
        [upper_left, upper_right, lower_right, lower_left]).reshape(-1, 2)
    corners = np.hstack(
...
def align_webtoon_image(img, device):
    ...
    bbox, landmark = pred[0]['bbox'], pred[0]['keypoints']
    if (bbox[-1] < face_thres) or (landmark[:, 2] < landmark_thres).any():
